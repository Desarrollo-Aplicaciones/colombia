...
if (isset($_GET['id_supply_order'])) {
  ...
  <html>
    ...
    <form action="icrassoc.php" method="POST" id="form_target">
      <div  style=" width: 100%; height: 95%;">
        <div style="width: 100%; margin: -21px 0px 0px 0px; font-family: verdana; font-size: 12px; color: dimgray;"><h1>Orden de suministros # <?php printf("%04d", $_GET['id_supply_order']); ?> | <?php if (isset($_GET['opcion'])) {
    if ($_GET['opcion'] == 'update') {
      ...
      echo '<b>Asociar</b>';
        ...
        <div style="float: left; width: 80%; height: 69%;; 
          ...
          <input type="hidden" value="<?php if (isset($_GET['id_supply_order'])) {
    echo $_GET['id_supply_order'];
  } ?>" name="id_supply_order" >
          <input type="hidden" value="1" name="add_products_order" >

          <input id="opcion" type="hidden" name="opcion" value="<?php if (isset($_GET['opcion'])) {
    echo $_GET['opcion'];
  } ?>">
          <input type="hidden" value="<?php if (isset($_GET['id_emp'])) {
    echo $_GET['id_emp'];
  } ?>" name="id_emp">
          <input type="hidden" value="<?php if (isset($_GET['lastname'])) {
    echo $_GET['lastname'];
  } ?>" name="lastname">
          <input type="hidden" value="<?php if (isset($_GET['firstname'])) {
    echo $_GET['firstname'];
  } ?>" name="firstname">
    ...
    <audio id="alert_irc" src="//<?php echo $host; ?>/sound/beam.wav" type="audio/wav"></audio> 
    <audio id="alert_irc_list" src="//<?php echo $host; ?>/sound/beam.wav" type="audio/wav"></audio> 
    <audio id="product_not_in_order" src="//<?php echo $host; ?>/sound/product_not_in_order.mp3" type="audio/mp3"></audio>
  ...
  <script type="text/javascript">
    ...
    $(document).ready(function () {
      ...
      // Ejecutar submit con el bot�n update    
      $("#save").click(function () {
        ...
        if ($('#prodsel').length) {
          ...
          if (icr != '')  // fin campo vacio
          {
            ...
            $.ajax({
              ...
              url: ruta,
              data: {
                "opcion": "<?php if (isset($_GET['opcion'])) {
    echo $_GET['opcion'];
  } ?>",
                "accion": "ajaxIcrAdd",
                "cod_icr": icr,
