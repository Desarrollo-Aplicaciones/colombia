...
if ( count($url_post) > 1 ) {
  ...
  $query_7 = mysqli_query($mysqli_1, " INSERT IGNORE INTO ps_product_supplier (id_product, id_supplier, product_supplier_reference, product_supplier_price_te, id_currency)
  ...
  $query_8 = mysqli_query($mysqli_1, "update ps_product pp INNER JOIN 
      (SELECT a1.`id_product` , a1.`id_supplier`, a1.`product_supplier_price_te` AS price 
        ...
        INNER JOIN 
          ( 
          SELECT MIN(psss.product_supplier_price_te ) AS pmin, psss.id_product , psss.id_supplier
          FROM `ps_product_supplier` psss 
          INNER JOIN ps_temp_product pstpo2 ON (psss.id_product = pstpo2.id_product AND psss.id_supplier = pstpo2.id_supplier ) 
          WHERE psss.id_product
            IN ( SELECT id_product
            FROM ps_temp_product
            WHERE flag != 'n'  )
          AND `product_supplier_price_te` > 0 
          GROUP BY psss.id_product
          ) b2 ON  
