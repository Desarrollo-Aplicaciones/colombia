	...
	public static function n_digit_random($digits){
		...
		public static function get_keys_open_pay($option_pay){
			...
			return array('id'=>$conn ['apilogin_id'],'public_key'=>$conn ['pseco_publickey'],'setsandboxmode'=> !(boolean)$conn ['produccion']);
		}
...
public static function set_cart_pay_process($id_cart = 0, $status = NULL)
{
	...
	if( $status_cart['in_pay'] && $status_cart['status']  != $status){
...
/**
 * Agrega un carrito a la sonda
 */
public static function add_sonda($id_cart,$payment = NULL,$pasarela = NULL)
{
	if((int)$id_cart == 0 || $payment == NULL || $pasarela == NULL)
		return 0;
	...
	return	$result = Db::getInstance()->insert('sonda_payu', array(
	                                           'id_cart' => (int)$id_cart,
	                                           'date_add'      => pSQL($date),
	                                           'interval' => (int)$interval,
	                                           'last_update' => pSQL($date),
	                                           'pasarela' => pSQL($pasarela),
	                                           ));
}
