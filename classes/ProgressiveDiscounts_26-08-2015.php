...
class ProgressivediscountsCore
{
	...
	public function validateLoggedUser() {
		...
		if ( $this->ObjectCart->id_customer != "" && $this->ObjectCart->id_customer != 0 && $resultsCustomer[0]['validCustomer'] != 0 && (int)$this->ObjectCart->id_customer == 11060 ) {
	...
	public function validateHistoryProgressiveDiscount() {
		...
		$querySearchHistory = "SELECT *
								FROM "._DB_PREFIX_."order_history_progressive_discounts
								WHERE id_customer = ".$this->idCustomer."
								AND id_product = ".$this->idProduct."
								AND id_progressive_discount = ".$this->idProgressiveDiscount."
								ORDER BY date_order DESC , id_order_history_progressive_discount DESC";
