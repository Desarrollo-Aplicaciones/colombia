	...
	{assign var="var_duplicada" value = false} 
		...
		{*} Mostrar Errores {*}
	  	{if isset($ERRORS_THIS_STEP) && !empty($ERRORS_THIS_STEP) && count($ERRORS_THIS_STEP) > 0}
					...
					{assign var="class_css" value = []}
					{foreach from=$ERRORS_THIS_STEP item=error}
						{if !$error['class']|in_array:$class_css}
							{$class_css[] = $error['class']}
						{/if}
					{/foreach}

			<span class="{if 'error'|in_array:$class_css} marcar_error{else}warning_orden{/if}" style="width:17px;" onclick="mostrar_errores(this)" messages="{foreach from=$ERRORS_THIS_STEP item=error}{$error['message']}|{/foreach}">
