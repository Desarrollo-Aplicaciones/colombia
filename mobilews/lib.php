    ...
    private function imgs_product($id_product=null)
    {
        ...
        if ($results = Db::getInstance()->ExecuteS($query)){
            foreach ($results as $value) {
                ...
                $array_img[] = _PS_BASE_URL_.__PS_BASE_URI__.$value['id_image']."-thickbox_default/img.jpg";
            }
         if($array_img!=NULL)
         ...
         $array_img[]=  _PS_BASE_URL_.__PS_BASE_URI__.'img/p/es-default-thickbox_default.jpg';
        return $array_img;
    }
...
. " and prod.active=1 AND prods.active=1 and prod.active=1 and prod.is_virtual=0 limit 1;";
 ...
 */
public function setOrderState($id_order,$state_order,$fecha,$latitud,$longitud , $tipo, $motivo,$user){
  $errors = array();
  ...
  if(!$this->change_state_order($id_order,$id_state_order,$user)){
      $errors[] = 'Error cambiando el estado la gu�a';
    }
  if(!$this->update_dete_add_order_history($id_order, $fecha)){
      $errors[] = 'Error actualizando la fecha de la orden';
    }    
...
static public function get_conf_status($alias){
 ...
 /**
   * Cambia el estado de una orden de salida
   */
protected function change_state_order($id_order,$id_state_order,$user){
  try{
    if((int) $id_order > 29452 ){
      $order = new Order((int) $id_order);  
      $current_state = $order-> getCurrentState();
      if($current_state != (int) $id_state_order && $current_state !=  Configuration::get('PS_OS_PAYMENT') && $current_state != Configuration::get('PS_OS_CANCELED'))
      { 
         $id_employee = $this->get_associate_carrier($id_order);
         if(empty($id_employee)){
          $id_employee = $this->get_external_client($user, 'systems_clients');
         }
         if(!empty($id_employee)){
          $order->setCurrentState((int) $id_state_order, $id_employee); 
         }else{
...
/**
 * Retorna el id del motorizado asociado a una orden
 */
protected function get_associate_carrier($id_order){
    $sql ="SELECT entity, id_entity FROM
          "._DB_PREFIX_ ."associate_carrier
          WHERE id_order =".(int)$id_order;
          $result = Db::getInstance()->ExecuteS($sql);
          if($result && strtolower($result[0]['entity']) == 'employee'){
            return (int)$result[0]['id_entity'];
            }else{
              return NULL;
            }          
}
 ...
 */
protected function get_external_client($user = NULL, $profile = 'systems_clients'){
    $sql =" SELECT emp.id_employee 
            ...
            WHERE prof.`name` = '".$profile."' AND emp.user_hash = '".$user."'";
          $result = Db::getInstance()->ExecuteS($sql);
          if($result && !empty($result[0]['id_employee'])){
            return (int)$result[0]['id_employee'];
            }else{
              return NULL;
            }          
}
/**
 * Cambia la feche del ultimo estado por la fecha real de entrega reportada por le m�vil de motorizado
 */
public function update_dete_add_order_history($id_order, $date_add){
    $sql = "UPDATE (SELECT MAX(id_order_history) as id_order_history FROM "._DB_PREFIX_ ."order_history
            WHERE id_order = ".$id_order." ) t1 INNER JOIN "._DB_PREFIX_ ."order_history oh ON (t1.id_order_history = oh.id_order_history) 
            SET oh.date_add = '".$date_add."';";
