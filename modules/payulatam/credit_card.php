...
class PayuCreditCard extends PayUControllerWS {
    ...
    public function process() {
      ...
      if ((Tools::getValue('numerot') 
          && !empty(Tools::getValue('numerot')) 
          && strlen(Tools::getValue('numerot')) > 13 
          && strlen((int) Tools::getValue('numerot')) < 17 
          && !empty(Tools::getValue('nombre')) 
          && !empty(Tools::getValue('codigot')) 
          && !empty(Tools::getValue('datepicker')) 
        && !empty(Tools::getValue('cuotas'))) 
        || (!empty(Tools::getValue('token_id')) 
          && !empty(Tools::getValue('openpay_device_session_id')) 
          && !empty(Tools::getValue('remember_tarjeta')))
      ) {
        ...
        if ($conn['nombre_pasarela'] == 'payulatam') {
          ...
          if ($response['code'] === 'ERROR') {
            ...
            $error_pay[] = $response;
              ...
              && $response['transactionResponse']['responseMessage'] != 'ERROR_CONVERTING_TRANSACTION_AMOUNTS') {
