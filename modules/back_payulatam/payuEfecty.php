...
"description":"'.$params[3]['description'].' Efecty",
"language":"es",
"notifyUrl":"'.$conf->urlv().'",
...
{
    $conf->failed_transaction(0.0, $this->context->customer->id, $data, $response, 'Efecty', 'code=ERROR', $this->context->cart->id);
...
$conf->pago_payu($order['id_order'], $this->context->customer->id, $data, $response, 'Efecty', $extras,$this->context->cart->id);
...
{
 $conf->failed_transaction(0.0, $this->context->customer->id, $data, $response, 'Efecty', 'code=PAYU_WAITING_PAYMENT', $this->context->cart->id);
}
else {
     $conf->failed_transaction(0.0, $this->context->customer->id, $data, $response, 'Efecty', 'code=Error indeterminado', $this->context->cart->id);
