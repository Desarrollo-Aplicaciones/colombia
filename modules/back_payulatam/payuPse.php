...
{
     $conf->failed_transaction(0.0, $this->context->customer->id, $data, $response, 'Pse', 'code=ERROR', $this->context->cart->id);
...
elseif ($response['code']=='SUCCESS') {
 ...
 $conf->pago_payu($order['id_order'], $this->context->customer->id, $data, $response, 'Pse', 'code=SUCCESS',$this->context->cart->id);
...
{
  $conf->failed_transaction(0.0, $this->context->customer->id, $data, $response, 'Pse', 'code=PAYU_WAITING_PAYMENT', $this->context->cart->id);
}
else {
      $messsage='Error indeterminado'; 
      $conf->failed_transaction(0.0, $this->context->customer->id, $data, $response, 'Pse', 'code=Error indeterminado', $this->context->cart->id);
     }
