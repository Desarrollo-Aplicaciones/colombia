...
{
      $conf->failed_transaction(0.0, $this->context->customer->id, $data, $response, 'Baloto', 'code=ERROR', $this->context->cart->id);
...
$conf->pago_payu($order['id_order'], $this->context->customer->id, $data, $response, 'Baloto', $extras, $this->context->cart->id);
...
{
 $conf->failed_transaction(0.0, $this->context->customer->id, $data, $response, 'Baloto', 'code=PAYU_WAITING_PAYMENT', $this->context->cart->id);
}
else {
     $conf->failed_transaction(0.0, $this->context->customer->id, $data, $response, 'Baloto', 'Error indeterminado', $this->context->cart->id);
