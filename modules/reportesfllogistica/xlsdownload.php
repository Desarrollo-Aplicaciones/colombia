...
if (isset($_GET['opc_sel']) ) {
    ...
    switch ($_GET['opc_sel']) {
        case 'consped':
			...
			IF ( ocr.reduction_percent IS NOT NULL AND ocr.reduction_percent != 0, CONCAT ( "Dto Porcentaje ", ocr.reduction_percent, " %" )  , 	
                            IF ( ocr.reduction_amount IS NOT NULL AND ocr.reduction_amount != 0, CONCAT( "Dto Monto", " $", ocr.reduction_amount ) , "")   ) AS tipo_descuento,
			IF ( ocr.reduction_percent IS NOT NULL AND ocr.reduction_amount IS NOT NULL AND ocr.reduction_product IS NOT NULL, 
