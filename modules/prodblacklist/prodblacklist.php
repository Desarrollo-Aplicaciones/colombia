...
function addBlackList() { 
    ...
    if (isset($_POST['id_emp']) && isset($_POST['motivo']) && isset($_POST['status']) && isset($_POST['id_product']) && isset($_POST['ref_product']) && isset($_POST['descripcion'])) {
        ...
        if($this->is_modifiable($_POST['ref_product']))
        {
            $flag1 = false;
            $flag2 = false;
            try {
         ...
         $this->output .= $this->displayError($this->l( utf8_encode('Este producto (' . $ref_product . '), no puede ser agregado a la lista negra, existen ordenes de salida pendientes de entrega o existen productos físicos en bodega. Para más información consulte con el área de logística.')));
         return false;
     }
 }
}
...
function enabledProd($reference_prod) { 
    ...
    if(!$this->isAllow($reference_prod)){
        $this->output .= $this->adminDisplayWarning($this->l('No tienes permisos para modificar el producto con la referencia (' . $reference_prod . ').'));
        return false;
    }
  ...
  $flag1 = false;
  $sql = 'UPDATE '._DB_PREFIX_.'product prod INNER JOIN '._DB_PREFIX_.'product_shop prods ON(prod.id_product = prods.id_product)
  SET prod.active = 1, prods.active = 1
  WHERE prod.reference = "'. $reference_prod.'"';


  $flag1 = mysqli_query($mysqli_1, $sql);

  if ($flag1) {
    $query = "DELETE FROM "._DB_PREFIX_."product_black_list  WHERE  reference='" . $reference_prod."'";
...
function search_black_list_products() {
    ...
    from ". _DB_PREFIX_ ."product_black_list black INNER JOIN ps_product prod on (black.reference=prod.reference)
    INNER JOIN ". _DB_PREFIX_ ."employee emp on(black.id_emp=emp.id_employee)
    INNER JOIN ". _DB_PREFIX_ ."product_lang plang ON(prod.id_product=plang.id_product)
    INNER JOIN ". _DB_PREFIX_ ."black_motivo mot ON(mot.id_black_motivo=black.motivo)
    WHERE prod.reference ='" . $reference_prod . "'";
...
function black_list_products() {
    $sql1 = 'SELECT COUNT(1) as total FROM 
    ps_product_black_list black INNER JOIN ps_product prod on (black.id_product=prod.id_product)
    ...
    // valores iniciales de paginación
        ...
        protected function is_modifiable($reference) {
            $query = "SELECT t1.lista_negra

            FROM
            (
             select IF(orders.current_state IN (5,6,8,16),    IF(icr.id_estado_icr != 2 , 'ok', 'no')   ,  'no' ) as lista_negra
             from ps_icr icr INNER JOIN ps_supply_order_icr s_order_icr ON(icr.id_icr = s_order_icr.id_icr )
             INNER JOIN ps_supply_order_detail s_order_d  ON ( s_order_icr.id_supply_order_detail = s_order_d.id_supply_order_detail)
             INNER JOIN ps_product prod ON (s_order_d.id_product = prod.id_product)
             LEFT JOIN ps_order_picking picking ON (icr.id_icr = picking.id_order_icr)
             LEFT JOIN ps_order_detail o_detail ON (picking.id_order_detail = o_detail.id_order_detail )
             LEFT JOIN ps_orders orders ON (o_detail.id_order = orders.id_order )
             WHERE prod.reference = '" . $reference . "' and icr.id_estado_icr in(2,3)
             GROUP BY lista_negra
             ) as t1 WHERE t1.lista_negra='no'";

$results = Db::getInstance()->ExecuteS($query);

if (!empty($results) && count($results) > 0 && $results[0]['lista_negra'] === 'no') {
    return false;
}
return true;
}

public function isAllow($reference_prod){
    $sql = "SELECT EXISTS (SELECT 1
                           FROM
                           ps_product_black_list
                           WHERE reference = '".$reference_prod."' AND id_emp = ".(int) $this->context->employee->id.") AS alow;";
