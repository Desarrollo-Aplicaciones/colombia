...
$reporte_icr['vencidos']['query'] =  "SELECT 
        ...
        orderdtl.name, p.reference, ml.`name` AS laboratorio, GROUP_CONCAT( DISTINCT sl.`name` ) AS proveedores,
...
/*********************************** ANITBIOTICOS VENDIDOS ************************************/

$reporte_icr['antibioticos']['query'] =  "SELECT * FROM antibioticos";
$reporte_icr['antibioticos']['sheet'] = 'Antibióticos Vendidos';
$reporte_icr['antibioticos']['title'] = ' Antibióticos Vendidos en las órdenes ';
...
/*********************************** ANITBIOTICOS VENDIDOS ************************************/
        ...
        foreach ( $reporte_icr as $iteracion => $query_text  ) {
         ...
         //echo("<br>".$iteracion);
                ...
                if ($results = Db::getInstance_slv()->ExecuteS($query_text['query'])) {
                    ...
                    if($iteracion == 'antibioticos'){
                        ...
                        $objPHPExcel->setActiveSheetIndex($sheet_number);
                            ...
                            $objPHPExcel->getActiveSheet()->getCell("A1")->setValue($query_text['title']);
                            ...
                            $objPHPExcel->getActiveSheet()->getCell("A3")->setValue(' ORDEN ');
                            $objPHPExcel->getActiveSheet()->getCell("B3")->setValue(' ICR ');
                            $objPHPExcel->getActiveSheet()->getCell("C3")->setValue(' LOTE ');
                            $objPHPExcel->getActiveSheet()->getCell("D3")->setValue(' FECHA VENCIMIENTO ');
                            $objPHPExcel->getActiveSheet()->getCell("E3")->setValue(' REFERENCIA ');
                            $objPHPExcel->getActiveSheet()->getCell("F3")->setValue(' PRODUCTO ');
                            $objPHPExcel->getActiveSheet()->getCell("G3")->setValue(' FECHA DE VENTA ');
                            $objPHPExcel->getActiveSheet()->getCell("H3")->setValue(' FACTURA ');

                            $objPHPExcel->getActiveSheet()->getStyle('A3:H3')->applyFromArray($style);
                            $objPHPExcel->getActiveSheet()->getStyle('A3:H3')->getBorders()->getAllBorders()->setBorderStyle(PHPExcel_Style_Border::BORDER_THICK);
                            ...
                            $line = $sheet_reg + 4;
                            $regshow++;
                            $objPHPExcel->getActiveSheet()->setCellValue('A'.$line, $dat_print['id_order']);
                            $objPHPExcel->getActiveSheet()->setCellValue('B'.$line, $dat_print['cod_icr']);
                            $objPHPExcel->getActiveSheet()->setCellValue('C'.$line, $dat_print['lote']);
                            $objPHPExcel->getActiveSheet()->setCellValue('D'.$line, $dat_print['fecha_vencimiento']);
                            $objPHPExcel->getActiveSheet()->setCellValue('E'.$line, $dat_print['reference']);
                            $objPHPExcel->getActiveSheet()->setCellValue('F'.$line, $dat_print['name']);
                            $objPHPExcel->getActiveSheet()->setCellValue('G'.$line, $dat_print['fecha_venta']);
                            $objPHPExcel->getActiveSheet()->setCellValue('H'.$line, $dat_print['factura']);
                            $sheet_reg ++;
                    }
                    else{
                        if ( $sheet_reg == 0 ) {
            ...
            $objPHPExcel->setActiveSheetIndex(0);
                if ( isset( $_GET ) && $_GET['descarga'] = 'si' ) {
...
$fileAttachment['name'] = "Reporte_ICR_Colombia.xls";
$fileAttachment['mime'] = "application/vnd.ms-excel";

$results = Db::getInstance_slv()->ExecuteS("SELECT * FROM ps_alerta_icr_correo WHERE activo = 1");
...
if ( Mail::Send(1, 'icr', 'Reporte automático generado de ICRs - Fechas de Vencimiento Colombia', array(), $correos, $nombres, "info@farmalisto.com.co", "Farmalisto icrs's Colombia", $fileAttachment) ) {
