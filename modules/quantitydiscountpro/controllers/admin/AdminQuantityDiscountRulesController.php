...
class AdminQuantityDiscountRulesController extends ModuleAdminController
{
    public function __construct()
    {
        ...
        $this->condition_selectors = array('group', 'product', 'category', 'country', 'attribute', 'zone', 'manufacturer', 'carrier', 'supplier', 'order_state', 'shop', 'gender', 'currency', 'state');
    ...
    public function processChangeStatusVal()
    {
        ...
        if (!$quantityDiscountRule->active) {
            QuantityDiscountRule::removeUnusedRulesByQuantityDiscountRule((int)$this->id_object);
        }
        if (!$quantityDiscountRule->update()) {
            $this->errors[] = Tools::displayError('An error occurred while updating rule.');
        }
    ...
    public function setMedia()
    {
        ...
        if (version_compare(_PS_VERSION_, '1.6', '>=')) {
            $this->addCSS(_MODULE_DIR_.'quantitydiscountpro/views/css/admin-theme.css');
        } else {
            $this->addCSS(_MODULE_DIR_.'quantitydiscountpro/views/css/admin-theme-15.css');
        }
    }
    ...
    public function initModal()
    {
        ...
        $this->context->smarty->assign(array(
            ...
            'module_name' => 'quantitydiscountpro',
        ));
    ...
    public function postProcess()
    {
        ...
        if (Tools::isSubmit('submitAddquantity_discount_rule') || Tools::isSubmit('submitAddquantity_discount_ruleAndStay')) {
            ...
            if (Tools::strlen(Tools::getValue('code_prefix')) > 5) {
                            ...
                            switch ((int)$id_type) {
                                ...
                                case 5:
                                    if (($form_values['customer_orders_amount_date_from'][$quantity_discount_rule_group][$quantity_discount_rule_group_rule] == '0000-00-00 00:00:00' || !$form_values['customer_orders_amount_date_from'][$quantity_discount_rule_group][$quantity_discount_rule_group_rule])
                                        ...
                                        && !(int)$form_values['customer_orders_amount_orders'][$quantity_discount_rule_group][$quantity_discount_rule_group_rule]) {
                ...
                foreach (array_keys($form_values['id_type']) as $quantity_discount_rule_action) {
                    ...
                    if ((int)$id_type) {
                        ...
                        switch ((int)$id_type) {
                             ...
                             */
                            case 5:
                                if (!Validate::isPercentage((float)str_replace(',', '.', $form_values['reduction_percent'][$quantity_discount_rule_action]))) {
            ...
            //ddd( $form_values['id_type'] );
            //ddd( ($actionBuyX && !$actionGetY) || (!$actionBuyX && $actionGetY) );
    ...
    protected function afterAdd($currentObject)
    {
        ...
        if ($form_values['id_type'] && array_filter($form_values['id_type'])) {
            foreach (array_keys($form_values['id_type']) as $quantity_discount_rule_action) {
                ...
                foreach (array_keys($definition['fields']) as $field) {
                    ...
                    if($form_values['id_type'][$quantity_discount_rule_action]==400){
                        $quantityDiscountRuleAction->reduction_percent_ciclos=implode(',',$form_values['reduction_percent_ciclos'][$quantity_discount_rule_action]);
                    }
        ...
        if (!$this->active) {
            QuantityDiscountRule::removeUnusedRulesByQuantityDiscountRule((int)$currentObject->id);
        }
    }
    ...
    public function processDelete()
    {
        if (Validate::isLoadedObject($object = $this->loadObject())) {
            QuantityDiscountRule::removeUnusedRulesByQuantityDiscountRule((int)$object->id);
    ...
    public function renderForm()
    {
        ...
        /** @var QuantityDiscountRule $current_object */
        ...
        if (Validate::isLoadedObject($current_object) && !$this->errors) {
    ...
    public function ajaxProcess()
    {
        ...
        if (Tools::isSubmit('newActionProduct') && Tools::getValue('action_id')) {
            $quantityDiscountRuleAction = new QuantityDiscountRuleAction();
            $quantityDiscountRuleAction->id_quantity_discount_rule_action = (int)Tools::getValue('action_id');
            $this->context->smarty->assign(
                array(
