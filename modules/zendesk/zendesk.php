...
class Zendesk extends Module
{
    ...
    public function __construct()
    {
        ...
        $this->module_key = '74f5cf75201c484610c98fcfa926478f';
    ...
    private function installApp()
    {
        ...
        if (is_array($apps->installations)) {
                ...
                if ((int)$installation->app_id == (int)$this->app_id) {
                    ...
                    if (empty($installation->settings->order_id_field_id)) {
                        ...
                        if (isset($ret->ticket_field->id)) {
                            $settings['order_id_field_id'] = (int)$ret->ticket_field->id;
                        }
                    }
                    ...
                    // Update settings
                    if ($need['to_update']) {
                        $ret = $this->api->updateApp((int)$installation->id, $settings);
                        if (!isset($ret->error)) {
        ...
        if ($need['to_install']) {
            ...
            $ret = $this->api->createTicketField($ticket_field);
            if (isset($ret->ticket_field->id)) {
                $install = $this->api->installApp((int)$this->app_id, (int)$ret->ticket_field->id);
                if (isset($install->id)) {
                ...
                $this->context->controller->errors[]  = $this->l('Can\'t process the creation of the user field.'.' '.($ret->description ? $ret->description : $ret->error));
            }
        }
    ...
    public function hookDisplayBackOfficeHeader()
    {
        ...
        } elseif (isset($this->context->controller) && $this->context->controller instanceof AdminCustomerThreadsController && !Tools::getvalue('forcelegacycontact') && !Tools::getValue('id_customer_thread') ) {
            Tools::redirectAdmin($this->context->link->getAdminLink('AdminZendesk'));
        }
    }
