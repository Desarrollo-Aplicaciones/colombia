...
foreach($results as $key => $value) {
	...
	if(isset ($status[$jsonResult['estado']]) && ( $status[$jsonResult['estado']] == 5 || $status[$jsonResult['estado']] == 19 || $status[$jsonResult['estado']] == 4) ) {
...
function get_mensajero_order($id_order) {
	$sql = "SELECT emp.id_employee, asoc.id_entity
				FROM ps_employee emp LEFT JOIN ps_associate_carrier asoc ON (emp.id_employee = asoc.id_employee)
