 ...
 $('"input[name=opc_ini]:radio"').change(function() {
   ...
   if(this.value != "cargatransciudad") {
    ...
    if(this.value != "envio_express") {
        $( "#xps_state" ).hide();
        $( "#divCiudadesExpress" ).hide();
    } else { 
        $('#xps_state').show( "slow" );
        $('#divCiudadesExpress').show( "slow" );
        expressEnabled();
    }
...
{* <form action="{$current}&token={$token}&configure=TransporteCiudades" id="target"  method="post"> *}
    <fieldset><legend>{$transciuLogo}</legend>
        ...
        <div class="opciones"> <input type="radio" name="opc_ini" value="envio_express"> Envío Express</div>
        <div id="xps_state"style="display:none;float: left; width: 100%; margin: 20px;"> <input type="checkbox" id="express" value="1" onchange="expressEnabled();" {if $expressEnabled} checked{/if}> Habilitar envío express
	        <div id="xps_options" style="display:none; float: left; width: 100%; margin: 20px;">
	        	...
	        	<p>Valor del envío express para compras inferiores a ${$Umbral}:
	        		<input type="number" id="xpsAbajo" onkeypress="return justNumbers(event);" disabled>
	        	</p>
	        	<p>Valor del envío express para compras superiores a ${$Umbral}:
        ...
        <div id="divCiudadesExpress" style="display:none;float: left; width: 100%; margin: 20px;"> 
            Ciudades con servicio de envió express:
            <a href="../modules/TransporteCiudades/GenerateReport.php">Presione Aquí Para Descargar Informe</a>
        </div>
