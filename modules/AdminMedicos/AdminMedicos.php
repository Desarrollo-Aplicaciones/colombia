...
class AdminMedicos extends Module {
    ...
    function __construct() {  //informacion del modulo
        ...
        if( strpos(strtolower($this->perfil_usuario['name']) , 'superadmin') !== false ) { //if (strtolower($this->perfil_usuario['name']) == 'superadmin') { // PARA SUPER ADMIN if( strpos(strtolower($this->perfil_usuario['name']) , 'superadmin') !== false )
            $this->permisos_basicos = 0;
        } elseif (  strpos(strtolower($this->perfil_usuario['name']) , 'jefe visitador' ) !== false ) { // PARA SUPERVISOR VISITADORES
        ...
        if ($result_old_datos = Db::getInstance()->ExecuteS($query_old_datos)) {
            ...
            if ($result_datos_up = Db::getInstance()->ExecuteS($query_datos_up)) {
                ...
                if ($result_del_especialidad = Db::getInstance()->ExecuteS($query_del_especialidad)) {
                    ...
                    //////--error_log($ins_espe_med);

                    if ($result_ins_espe_med = Db::getInstance()->ExecuteS($ins_espe_med)) {

                        //INSERTAR LOG DE COMISIONES ENTREGADAS A VISITADORES
                        //////--error_log("....----- permisos basicos -> ".$this->permisos_basicos);
                        //////--error_log($result_old_datos[0]['id_visitador'] .'<- visitador ->'. $visitador);
...
// Muestra el formulario de actualización de cupones médicos
    ...
    public function update_cart_medic_rueles(){
        $sql ="UPDATE " . _DB_PREFIX_ . "medico medic LEFT JOIN " . _DB_PREFIX_ . "employee visitador ON (medic.id_visitador = visitador.id_employee)
                INNER JOIN " . _DB_PREFIX_ . "medico_rule medr ON (medic.id_medico = medr.id_medico)
                INNER JOIN " . _DB_PREFIX_ . "cart_rule rule ON (medr.id_cart_rule = rule.id_cart_rule)
                SET rule.reduction_percent =
                CASE
                WHEN ISNULL(medic.id_visitador) THEN
                0 
                WHEN !ISNULL(medic.id_visitador) THEN
                5
                END;";
         ...
         if ($result = Db::getInstance()->Execute($sql)) {
                ...
                $this->errores .= "<br> No se pudo actualizar los cupones de médicos." . "<br> Mensaje Error: " . Db::getInstance()->getMsgError();
                return false;
            }
