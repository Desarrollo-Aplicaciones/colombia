...
<script type="text/javascript">
  var INIT_INTERVALS = {$inicio_intervalos};
  var MAX_HOURS_DELIVERED = {$numero_horas};
  var INIT_HOUR_DELIVERE = {$hora_inicio};
  var END_HOUR_DELIVERE = {$hora_fin};
  $( document ).ready(function() {
    // Hora de Entrega Productos 
    ...
    // Agrega las opciones iniciales
    var optionsDate = [];
    optionsDate.push('<option value="">Seleccione un Día</option>');

    optionsDate.push('<option value="' + today.toISOString().substring(0, 10) + '">Hoy ' + weekday[today.getDay()] + ' ' + today.getUTCDate() + ' ' + months[today.getMonth()] + '</option>');

    optionsDate.push('<option value="' + tomorrow.toISOString().substring(0, 10) + '">Mañana ' + weekday[tomorrow.getDay()] + ' ' + tomorrow.getUTCDate() + ' ' + months[tomorrow.getMonth()] + '</option>');

    optionsDate.push('<option value="' + afterTomorrow.toISOString().substring(0, 10) + '">El ' + weekday[afterTomorrow.getDay()] + ' ' + afterTomorrow.getUTCDate() + ' ' + months[afterTomorrow.getMonth()] + '</option>');

    $("#date_delivered").html(optionsDate.join(''));

    // Agrega las opciones de hora
    var optionsHour = [];
    optionsHour.push('<option value="">Seleccione Hora</option>');

    if (today.getHours() % INIT_INTERVALS) {
      today.setHours(today.getHours() - 1); 
    }

    today.addHours(INIT_INTERVALS);

    for (i = 1; i <= MAX_HOURS_DELIVERED; i++) {
      var hourFrom = today.getHours();
      if ( today.getHours() < END_HOUR_DELIVERE && today.getHours() >= INIT_HOUR_DELIVERE ) {
        today.addHours(INIT_INTERVALS);
      }else {
        today.setDate(today.getDate() + 1);
        today.setHours(INIT_HOUR_DELIVERE);
  
        continue;
      }
      var timeOption = hourFrom + ':00 a ' + today.getHours() + ':00';
      optionsHour.push('<option value="' + timeOption + '" data_day="' + today.getDate() + '">' + timeOption + '</option>');
    }
    $("#hour_delivered").html(optionsHour.join(''));

    // Oculta los horarios
    $('#hour_delivered option[data_day]').hide();

    $( "#date_delivered" ).change(function() {
      $('#hour_delivered option[data_day]').hide();
      var thisVal = $(this).val().split("-")[2];
      if ( typeof thisVal != 'undefined' ) {
        // Muestra los horarios del día seleccionado
        $('#hour_delivered option[data_day="'+thisVal+'"]').show();
      }
    });
