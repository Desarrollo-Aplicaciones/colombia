...
{literal}
	<script>
		...
		function changeBtnActive( idActive, idDisable ){
			$(idActive).removeClass("disable");
			$(idDisable).removeClass("disable");

			$(idActive).addClass("active");	
			$(idDisable).addClass("disable");
			
			$('#ctn-input-cupon').stop().slideDown();
			if ( idActive == "#btncupon"){
				$('input#radio-md').prop("checked", false);
				$('input#radio-cupon').prop("checked", true);
				$('#discount_name').attr('placeholder','Ingrésa tu cupón');
			}
			else {
				$('input#radio-cupon').prop("checked", false);
				$('input#radio-md').prop("checked", true);
				$('#discount_name').attr('placeholder','Ingrésa el nombre');
			}
		}
	...
	<script>
		function mouse_overd( objeto , idCotainer ){
	        ...
	        if(objeto == "div7") {
			    ...
			    document.getElementById("textradiobaloto").style.color='#646464';
			    document.getElementById("textradiobaloto").style.fontWeight='600';
			    $('[value=div7]').attr("checked", true);
			}
			else if(objeto == "div8") {
				clearDivs();
			    ...
			    document.getElementById("textpse").style.color='#646464';
			    document.getElementById("textpse").style.fontWeight='600';
			    ...
			    $('[value=div6]').attr("checked", true);
			}
	        else if(objeto == "div5") {
	        	clearDivs();
			    document.getElementById("botoncitosubmit").onclick = submitforms5;
			    document.getElementById("textradioefecty").style.color='#646464';
			    document.getElementById("textradioefecty").style.fontWeight='600';
			    $('[value=div5]').attr("checked", true);
			}
	        else if(objeto == "div10") {
	        	clearDivs();
			    document.getElementById("botoncitosubmit").onclick = submitforms5;
			    document.getElementById("textradiodatafono").style.color='#646464';
			    document.getElementById("textradiodatafono").style.fontWeight='600';
			    $('[value=div10]').attr("checked", true);
			}
	    }
	    ...
	    function clearDivs() {
	     	pest = Array();
	     	rb = Array();
	     	pest[1] = "#textradioefecty";
	    	pest[2] = "#textradiocontrae";
	    	pest[3] = "#textradiobaloto";
	    	pest[4] = "#texttarjeta";
	    	pest[5] = "#textpse";
								...
								<div  class="resumen">Resumen:</div>
									<div class="columna1">
														...
														<div class="cart_total_price">
															{foreach from=$discounts item=discount name=discountLoop}
																<div class="cart_discount {if $smarty.foreach.discountLoop.last}last_item{elseif $smarty.foreach.discountLoop.first}first_item{else}item{/if}" id="cart_discount_{$discount.id_discount}">
																	<div class="descripcion">
																		<span class="celeste">Beneficio*: {* $discount.name *} </span>
																	</div>
										...
										<div class="cont_cupones">
		                        			{if sizeof($discounts)}
		                        				<div class="suc_dis">Tu cupón se registró con éxito
		                        				</div>
		                        			{else}
		                        				{if $voucherAllowed}
		                        					{if isset($errors_discount) && $errors_discount}
		                       						...
		                       						<form action="{if $opc}{$link->getPageLink('order-opc', true)}{else}{$link->getPageLink('order', true)}{/if}" method="post" id="voucher">
		                       							<fieldset id="addDiscount">
		                       								<p class="label_cupon">
		                       									¿Tienes algún cupón de beneficio o conoces el nombre de tu médico?
		                       								</p>
		                       								<div class="ctn-buttons-cupon">
		                       									<input id="radio-md" type="radio" name="type_voucher" value="md" style="display:none;">
		                       									<input id="radio-cupon" type="radio" name="type_voucher" value="cupon" style="display:none;">
		                       									<div class="btn-cupon" id="btnmedico" onclick="changeBtnActive( '#btnmedico' , '#btncupon' )"> MÉDICO </div>
		                       									<div class="btn-cupon" id="btncupon"  onclick="changeBtnActive( '#btncupon' , '#btnmedico')"> CUPÓN </div>
		                       								</div>
		                       								<div class="cont_input_cupon" id="ctn-input-cupon" style="display:none">
			                       								<input type="text" class="discount_name" id="discount_name" name="discount_name" value="{if isset($discount_name) && $discount_name}{$discount_name}{/if}" placeholder="Ingrésa tu cupón"/>
										...
										<div class="contenedorGris">
											...
											{/if}
													<div id="HOOK_PAYMENT">
														{literal}
															<style type="text/css">
																#HOOK_PAYMENT div div{float: lefties;}
															</style>
														{/literal}
														{$HOOK_PAYMENT}
													</div>
											...
											{/if}
											{* <div id="imagenseguridad">
												<img src="{$img_dir}authentication/seguridad.jpg" />
											</div> *}
									</div>
								</div>
							</div>
...
<script type="text/javascript" src="{$base_dir_ssl}js/bsn.AutoSuggest_2.1.3.js" charset="utf-8"></script>
