...
//  }
  ...
  public function ValidateFechaVencEntrada() {
    // self::debug_to_console("Entro ValidateFechaVencEntrada OVERRIDE");
      ...
      if (isset($retorno[0]['tamanofec']) && $retorno[0]['tamanofec'] == 0) {
        ...
        if ($retorno_regexfec = DB::getInstance()->executeS($query_regexfec)) {
          ...
          if (isset($retorno_regexfec[0]['regexfec']) && $retorno_regexfec[0]['regexfec'] == 0) {
              ...
              if (isset($retorno_diainvalid[0]['diainvalid']) && $retorno_diainvalid[0]['diainvalid'] == 0) {
                ...
                if ($retorno_fecinvalid = DB::getInstance()->executeS($query_fecinvalid)) {
                  ...
                  if (isset($retorno_fecinvalid[0]['fecinvalid']) && $retorno_fecinvalid[0]['fecinvalid'] == 0) {
                    // self::debug_to_console("RETURN ValidateFechaVencEntrada: TRUE");
                    return true;
                  } else {
                    ...
                    // self::debug_to_console("1.- Algunas fechas de vencimiento de algunos ICR no es mayor a la actual. Cantidad de fechas erróneas ( " . $retorno_fecinvalid[0]['fecinvalid'] . " ). Icrs: " . $retorno_fecinvalid[0]['icrs']);
                    return false;
                  }
                } else {
                  $this->errores_cargue[] = "No se puede comprobar que fecha de vencimiento de los ICR sea mayor a la actual.";
                  // self::debug_to_console("2.- No se puede comprobar que fecha de vencimiento de los ICR sea mayor a la actual.");
                  return false;
                }
              } else {
                ...
                // self::debug_to_console("3.- Algunas fechas de vencimiento de algunos ICR es errónea (Inexistente). Cantidad de fechas erróneas ( " . $retorno_diainvalid[0]['diainvalid'] . " ). ");
                return false;
              }
            } else {
              $this->errores_cargue[] = "No se puede comprobar la validez de la fecha de vencimiento de los ICR.";
              // self::debug_to_console("4.- No se puede comprobar la validez de la fecha de vencimiento de los ICR.");
              return false;
            }
          } else {
            ...
            // self::debug_to_console("5.- El formato de fecha de vencimiento de algunos ICR es erróneo, debe ser YYYY-MM-DD. Cantidad de fechas erróneas ( " . $retorno_regexfec[0]['regexfec'] . " ). Icrs: " . $retorno_regexfec[0]['icrs']);
            return false;
          }
        } else {
          $this->errores_cargue[] = "No se puede validar formato de fecha, de la fecha de vencimiento de los ICR.";
          // self::debug_to_console("6.- No se puede validar formato de fecha, de la fecha de vencimiento de los ICR.");
          return false;
        }
      } else {
        ...
        // self::debug_to_console("7.- Se ha presentado error en el tamaño de fecha de vencimiento de algunos ICR (10 caracteres). Cantidad de fechas erróneas ( " . $retorno[0]['tamanofec'] . " ). Icrs: " . $retorno[0]['icrs']);
        return false;
      }
    } else {

      $this->errores_cargue[] = "No se puede validar el tamaño de la fecha de vencimiento de los ICR.";
      // self::debug_to_console("8.- No se puede validar el tamaño de la fecha de vencimiento de los ICR.");
      return false;
    }
  }
  ...
  public function loadicrentrada($path_file_load_db) {
    ...
    // self::debug_to_console($path_file_load_db, "Entro loadicrentrada y el  path_file_load_db OVERRIDE: ");
    $mysqli_1 = mysqli_init();
    mysqli_options($mysqli_1, MYSQLI_OPT_LOCAL_INFILE, true);
    ...
    if (mysqli_connect_errno()) {
      ...
      $this->errores_cargue[] = "Conexión fallida: %s\n mensaje error: " . mysqli_connect_error();
      // self::debug_to_console($this->errores_cargue, "this->errores_cargue[] RETURN FALSE");
      return false;
    }
    ...
    if (!mysqli_query($mysqli_1, "TRUNCATE TABLE ps_tmp_cargue_entrada_icr")) {
      ...
      // self::debug_to_console($this->errores_cargue, "this->errores_cargue[] RETURN, TRUNCATE TABLE ps_tmp_cargue_entrada_icr, FALSE");
      return false;
    }
    ...
    // self::debug_to_console($this->errores_cargue, "RETURN loadicrentrada TRUE");
      return true;
    }
    return false;
  }
