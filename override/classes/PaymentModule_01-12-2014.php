...
//valida si tiene doble descuento tanto en la entrada de direccion como en cupon de envio adentro del back office
                    ...
                    //validacion de direccion correspondiente a oficina central
                    $resultvalidationaddressof = $order->validaraddressOficina($address2);
                    $validacionfreeshippingof = false;

                    if ($resultvalidationaddressof && $address == 'BOGOTá, D.C.') {
                        //VALIDACION POR ALIAS -------- && $addre == 'FARMALISTO'
                        $restoasignacion = 0;
                        if ($opera != 1  && $cartruller == "") {
                            $restoasignacion = 1;
                        } elseif (empty($opera) && $cartruller != "") {
                            $porcentaje = $order->total_products_wt / 100;
                            $porcentaje = $porcentaje * $cartruller;
                            $restoasignacion = 1;                          
                        }

                        if ($restoasignacion == 1){
                            $validacionfreeshippingof = true;
				...
				if(Context::getContext()->cookie->check_xps)
                    ...
                    if ($resultvalidationaddressof) {
                        if (empty($opera) && $address == 'BOGOTá, D.C.') {
                            //VALIDACION POR ALIAS -------- if ($addre == 'FARMALISTO') {
                                if ($cartruller != " ") {
