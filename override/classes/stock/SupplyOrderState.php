...
class SupplyOrderState extends SupplyOrderStateCore
{
	public static function getSupplyOrderStates($id_state_referrer = null, $id_lang = null)
	{
		...
		if (!is_null($id_state_referrer))
		{
			...
			if(strpos($profile->name[1], '(edit)') === FALSE) // si en el nombre del perfil actual no contiene la etiqueta "(edit)", entonces no se permite al usuario actual habilitar la edición de la orden 
