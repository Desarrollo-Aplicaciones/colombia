...
public static function find($id_lang, $expr, $page_number = 1, $page_size = 1, $order_by = 'position2',
		$order_way = 'desc', $ajax = false, $use_cookie = true, Context $context = null)
	{
		...
		if ($ajax)
		{
			...
			/*$query_psql = "SELECT si.id_product, SUM(si.weight) AS peso, ( 100 - ( ( search.levenshtein(sw.word, tw.palabra) ) * 100)/ length(sw.word) ) AS porce FROM search.ps_search_index si
				INNER JOIN search.ps_search_word sw ON ( sw.id_word = si.id_word )
				INNER JOIN (SELECT row_number() OVER() AS orden, UNNEST(STRING_TO_ARRAY('".$listword."', ';')) palabra ) tw 
					ON ( ( 100 - ( ( search.levenshtein(sw.word, tw.palabra) ) * 100)/ length(sw.word) ) >= 70 )
				GROUP BY ( 100 - ( ( search.levenshtein(sw.word, tw.palabra) ) * 100)/ length(sw.word) ), si.id_product, tw.orden
				ORDER BY ( 100 - ( ( search.levenshtein(sw.word, tw.palabra) ) * 100)/ length(sw.word) ) DESC, tw.orden, peso DESC";
				*/
			...
			$sql = "SELECT DISTINCT p.id_product, pl.name pname, cl.name cname, cl.link_rewrite crewrite, pl.link_rewrite prewrite, position2.peso, IF(position2.porc = 100 ,".$reference_load.",0) AS ref_fnd, GROUP_CONCAT(DISTINCT im.id_image SEPARATOR ',') AS imgs, p.id_tax_rules_group AS taxgr, t.rate,
				CASE p.id_tax_rules_group
					...
					ELSE IF(t.rate IS NOT NULL, ROUND(p.price + (p.price * t.rate/100),0), ROUND(p.price,0) )
					END
					AS price
				FROM ps_product p
				...
				if ($alenum != 0 && $result = $db->executeS($sql)) {
					...
					//$loggin->lwrite("Search_ajax", "log_busquedas.txt", print_r($result,true));
					$array_full = array();
					$cantnum = 0;
					...
					foreach ($result as $value) {						
						...
						if ( $value['imgs'] != NULL ) {				
								...
								$array_full[$cantnum]['imgs'] = _PS_BASE_URL_
								. __PS_BASE_URI__
								. 'img/p/'
		...
		/*$sql = "SELECT si.id_product, SUM(si.weight) AS peso, ( 100 - ( ( search.levenshtein(sw.word, tw.palabra) ) * 100)/ length(sw.word) ) AS porce FROM search.ps_search_index si
				INNER JOIN ps_search_word sw ON ( sw.id_word = si.id_word )
				INNER JOIN (SELECT row_number() OVER() AS orden, UNNEST(STRING_TO_ARRAY('".$listword."', ';')) palabra ) tw 
					ON ( ( 100 - ( ( search.levenshtein(sw.word, tw.palabra) ) * 100)/ length(sw.word) ) >= 70 )				
				GROUP BY ( 100 - ( ( search.levenshtein(sw.word, tw.palabra) ) * 100)/ length(sw.word) ), si.id_product, tw.orden
...
//                }
 ...
 //exit;
 //error_log("|".$sql."|", 0);
 // error_log($sql, 3, "/home/ubuntu/modules/Ordenes_de_Suministros_FTP/my-errors.log");
		$result = $db->executeS($sql);
