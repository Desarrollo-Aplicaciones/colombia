...
class Cart extends CartCore {
    ...
    public static function addCartMedico($id_cart, $id_medico, $texto = false) {
        if($texto){
            $sql_max = "SELECT MAX(id_medico) as max_medico FROM "._DB_PREFIX_."medico WHERE id_visitador = 20";
            $result_max = Db::getInstance()->getRow($sql_max);
            $result_max["max_medico"]++;
            $sql_medico = "INSERT INTO "._DB_PREFIX_."medico (`id_medico`, `nombre`, `cedula`, `id_visitador`) VALUES ('".$result_max["max_medico"]."', '".$id_medico."', '".$result_max["max_medico"]."', '20');";
    ...
    public function getTotalShippingCost($delivery_option = null, $use_tax = true, Country $default_country = null, $express = false) {
        ...
        if ((isset(Context::getContext()->cookie->check_xps) && Context::getContext()->cookie->check_xps) || (isset(Context::getContext()->cart->check_xps) && Context::getContext()->cart->check_xps) || Tools::getValue('express')) {
        ...
        if ((!isset(Context::getContext()->cookie->check_xps) || !Context::getContext()->cookie->check_xps) && isset(Context::getContext()->cookie->entrega_nocturna) && Context::getContext()->cookie->entrega_nocturna === 'enabled') {
...
//    $producto_rx3 = false; //Inicializo la variable en falso, para usarla de banderita. :)
        if (isset($resultado[0]['id_carrier'])) {
            ...
            $val_total = 0;
            // error_log("PRIMERA: producto_rx3: " . $this->producto_rx3);
            if (Context::getContext()->cart->_products) {
                ...
                foreach (Context::getContext()->cart->_products as $productico) {
                    ...
                    if ((stripos($productico['name'], "rx3")) !== false) {
                        $this->producto_rx3 = true;
                        // $this->CONTADOR++;
                        // error_log("CONTADOR: " . $this->CONTADOR);
                        break;
                    } else {
                        $this->producto_rx3 = false;
                    }
                }
            }
            // error_log("PRIMERA: REULT producto_rx3: " . $this->producto_rx3);
        ...
        //echo $val;
        if ($this->producto_rx3 && !(in_array($resultado[0]['id_city'], $ciudades_excluidas) )) {
    ...
    public function getPackageShippingCost($id_carrier = null, $use_tax = true, Country $default_country = null, $product_list = null, $id_zone = null, $express = false) {
        if (!is_null($product_list)) {
            foreach ($product_list as $key => $value) {
                // error_log("ID DE PRODUCTO: " . $value['id_product']);
                if ($value['id_product'] == 39473 || $value['id_product'] == 39474) {
                    // error_log("ENTRO ID: " . $value['id_product']);
                    return 0;
                }
            }
        }
        ...
        if ((isset(Context::getContext()->cookie->check_xps) && Context::getContext()->cookie->check_xps) && (!isset(Context::getContext()->cookie->entrega_nocturna) || Context::getContext()->cookie->entrega_nocturna === 'disabled')) {
...
//      $producto_rx3 = false; //Inicializo la variable en falso, para usarla de banderita. :)
        ...
        foreach (Context::getContext()->cart->_products as $key => $productico) {
            ...
            if ((stripos($productico['name'], "rx3")) !== false) {
                $this->producto_rx3 = true;
//                $this->CONTADOR2++;
                // // error_log("CONTADOR2: " . $this->CONTADOR2);
                break;
            } else {
                $this->producto_rx3 = false;
            }
        ...
        $iva_envio_orden = Configuration::get('IVA_ENVIO_ORDEN');
        $valtot_tax = (round((round($val_total * ( 1 + ($iva_envio_orden / 100) )) / 100) * 2, 0) / 2 ) * 100; //valor con impuesto 16% y redondeado
        // // error_log("Val_total: " . $val_total . ", Resultado[0]['delimiter2']: " . $resultado[0]['delimiter2']);
        if ($val_total > $resultado[0]['delimiter2']) { // si total de compra es mayor al valor para no cobrar envio
            // // error_log("si total de compra es mayor al valor para no cobrar envio: Val_total: " . $val_total . ", Resultado[0]['delimiter2']: " . $resultado[0]['delimiter2']);
            $shipping_cost = 0;
        }

        // si la direccion de envio es la oficina de farmalisto, se toma como 0 el costo de transporte
        $validateaddress = $this->ValidationAddressFarmalisto();
        // // error_log("Validateaddress: " . (int) $validateaddress);
        if ($validateaddress) {
            $shipping_cost = 0;
        }
        ...
        // si existe cupon con envio gratuito, se toma como 0 el costo de transporte
        $cartRules = $this->getCartRules();
        // // error_log("CartRules: " . $cartRules . ", CartRules[0]['free_shipping']: " . $cartRules[0]['free_shipping']);
        ...
        //echo $shipping_cost;
        // // error_log("Producto_rx3: " . (int) $this->producto_rx3 . ", ID CIUDAD: " . $resultado[0]['id_city'] . ", Ciudades_excluidas; " . $ciudades_excluidas);
        if ($this->producto_rx3 && !(in_array($resultado[0]['id_city'], $ciudades_excluidas) )) {
            // // error_log("CIUDAD EXCLUIDA: " . $resultado[0]['precio_k_add']);
            return $resultado[0]['precio_k_add'];
        }
        // // error_log("FINAL: " . $shipping_cost);
...
//----------------------  INICIO  ----------------------------   NEVERAS   -------------------------------------  EWING  -------------//
        ...
        if (/* $this->id_customer == 250 && */!is_null($products) && count($products) > 0 && ( isset(debug_backtrace()[1]['class']) && debug_backtrace()[1]['class'] != "CartRule" && debug_backtrace()[1]['function'] != "getContextualValue") && ( isset(debug_backtrace()[1]['class']) && debug_backtrace()[1]['class'] != "CartRuleCore" && debug_backtrace()[1]['function'] != "checkValidity")) {
...
// error_log("TOTAL ORDEN : ".Tools::ps_round((float) $order_total, 2));
    ...
    public function add_total_tax($totalTax) {
        ...
        $rs = Db::getInstance()->update('cart', array(
          'total_tax' => $totalTax,
            ), 'id_cart = ' . (int) $this->id);
    }
    ...
    public function validationaddressfarmalisto() {
        ...
        // error_log("VALIDAR validateaddress: " . (int) $validateaddress);
