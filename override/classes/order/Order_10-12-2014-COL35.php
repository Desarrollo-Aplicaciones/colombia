...
class Order extends OrderCore {  
	...
	public function addOrderPayment($amount_paid, $payment_method = null, $payment_transaction_id = null, $currency = null, $date = null, $order_invoice = null)
	{
	    ...
	    if ($descuento != "" && $descuento != 0){
	    	...
	    	//$addressact = new Address($this->id_address_delivery);
	    	//$address_delivery = strtoupper($addressact->address1);
	    	//$address_city = strtoupper($addressact->city);

			//$respuestavalidationaddress = $this->validaraddressOficina($address_delivery);

			//if($respuestavalidationaddress && $address_city == 'BOGOTá, D.C.'){
				//$totalenvio = 0;
			//} else {
	...
	public function validaraddressOficina($addressactual){

        $addressoficina = array(
        ...
        $validationaddressOfc = false;

        foreach ($addressoficina as $address) {
            if ($addressactual == $address){
                $validationaddressOfc = true;
            }
        }

        return $validationaddressOfc;
    }
}