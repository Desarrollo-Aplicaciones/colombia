...
class AdminOrdersController extends AdminOrdersControllerCore {
  ...
  public function __construct() {
    ...
    $this->_select = '
		...
		IF((SELECT COUNT(so.id_order) FROM `' . _DB_PREFIX_ . 'orders` so WHERE so.date_add >= "'.date("Y").'-07-01 00:00:00" AND so.id_customer = a.id_customer) > 1, 0, 1) as `new`';
    $this->_join = '
    ...
    $this->_where = ' AND a.date_add >= "'.date("Y").'-07-01 00:00:00"';
    ...
    if (Tools::getIsset('controller') && Tools::getValue('controller') == 'AdminOrders' && Tools::getIsset('vieworder') && Tools::getIsset('id_order') && Tools::getIsset('ajax') && Tools::getIsset('option_jax') && Tools::getIsset('token')) {
      ...
      if ($option_ajax === 'transportador_fm') {
        ...
        if ($this->opcionTransportista($_GET)) {
          ...
          if ($order->current_state == 22) {
            ...
            if (count($resultStateOrder) > 0 && !empty($resultStateOrder[0]['id_order_state'])) {
              ...
              $fecha_alistado = date("Y-m-d");
              $date_add = strtotime($order->date_add);
              $fecha_creacion = date("Y-m-d", $date_add);
              $hora_creacion = date("His", $date_add);
        ...
        //print_r($hora);    exit;         // print_r($fecha_entrega); 
              $url_insercion = $server . '/restfarmalisto/servicio_rest/MantieneReceptor/insertar_visita/' . $pedido . '/' . $fecha_alistado . '/' . $hora_entrega . '/' . $ciudad . '/' . $direccion . '/' . $doc_cliente . '/' . $nom_cliente . '/' . $telefono . '/' . $observacion . '/' . $doc_mensajero . '/' . round($total_paid) . '/' . $payment . '/' . $fecha_creacion . '/' . $hora_creacion . '/' . $fecha_entrega;
       // print_r($url_insercion);
        ...
        if ($this->save_opcion_cancelacion($_GET)) {
          ...
          if ($_GET['opcion_cancelacion'] != 4) {
            ...
            Mail::Send(
            1, 'canceled', 'Tu pedido ha sido cancelado. Por favor califica nuestro servicio', $template_vars, $this->context->customer->email, ( $this->context->customer->firstname . " " . $this->context->customer->lastname), Configuration::get('PS_SHOP_EMAIL'), Configuration::get('PS_SHOP_NAME')
            );
          }
      ...
      if ($this->tabAccess['edit'] === '1') {
        ...
        elseif (!Validate::isTrackingNumber(Tools::getValue('tracking_number')))
          ...
          if ($order_carrier->update()) {
            ...
            if (@Mail::Send((int) $order->id_lang, 'in_transit', Mail::l('Package in transit'), $templateVars, $customer->email, $customer->firstname . ' ' . $customer->lastname, null, null, null, null, _PS_MAIL_DIR_, true, (int) $order->id_shop)) {
          ...
          if ($productList || $customizationList) {
            ...
            if (Tools::isSubmit('generateCreditSlip') && !count($this->errors)) {
              if (!OrderSlip::createOrderSlip($order, $full_product_list, $full_quantity_list, Tools::isSubmit('shippingBack')))
                ...
                @Mail::Send(
                (int) $order->id_lang, 'credit_slip', Mail::l('New credit slip regarding your order', $order->id_lang), $params, $customer->email, $customer->firstname . ' ' . $customer->lastname, null, null, null, null, _PS_MAIL_DIR_, true, (int) $order->id_shop
                );
              }
            }
            // Generate voucher
