...
return array();
	...
	public function getProdCategories($ids_categories, $page_number=1,$page_size=10, $order_way=null,$order_by=null)
	{
		...
		{  
			...
			$total_filas = 0;
			$query = " SELECT COUNT(prod.id_product) total, lower(cat_prodl.name) AS title
			...
			$query = " SELECT prod.id_product, prod.reference, lower(prodl.`name`) AS name, cat_prodl.id_category as id_cat, lower(cat_prodl.`name`) as cat,
			m.id_manufacturer AS id_lab, lower(m.`name`) AS lab,
			CASE prod.id_tax_rules_group
			WHEN  0 THEN ROUND(prod.price,0)
			ELSE IF(taxr.id_tax IS NOT NULL, ROUND( prod.price + (prod.price * tax.rate/100),0), ROUND(prod.price,0) )
			END
			AS `price`,			
			...
			if ( $results = Db::getInstance()->ExecuteS($query) ) {
				...
				foreach ( $results as $value ) {
					...
					$array_prod[] = array(
					                      'id' => (int) $value['id_product'],
					                      'reference' => (string) $value['reference'],
					                      'name' =>  htmlentities( strtolower(strip_tags($value['name'])) ),
...
public function getProduct($id) {
	...
	$query = "SELECT p.id_product AS id, lower(pl.`name`) AS name, lower(pl.description_short) AS 'desc', GROUP_CONCAT( DISTINCT im.id_image ORDER BY im.cover DESC SEPARATOR ',') AS imgs,
	ROUND( ps.price + IF ( t.rate IS NULL , 0 , ps.price * ( ( t.rate/100 ) ) ), 0) AS price,
...
private function createOrder($method,$state) {
    ...
    public function add_image($files,$option){
    	...
    	error_log('< - add_image - |'.print_r($files,true).'|>', 0);
...
//.'|'.end(explode('.',$files['name']));
    ...
    public function get_order_datail($id_order){
    	$sql = "		SELECT p.`name`, od.product_quantity, od.product_price,od.total_price_tax_incl
    	FROM `"._DB_PREFIX_."order_detail` od
    	LEFT JOIN `"._DB_PREFIX_."product_lang` p ON (p.id_product = od.product_id)
    ...
    public function get_traker_order($id_order){
    	...
    	$sql = "SELECT ol.id_order_state, ol.`name` as order_state, cu.firstname, cu.lastname, o.id_order, o.reference, asoc.entity,
    	CASE asoc.entity
    	WHEN  'employee' THEN CONCAT(emp.firstname,' ' , emp.lastname)
    	WHEN  'Carrier' THEN carri.`name`
    	ELSE 'N/A'
    	END AS carrier, ad.address1, ad.address2, ad.postcode 
