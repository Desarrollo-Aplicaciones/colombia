...
public function setAccount($arg){
	...
	$customer->lastname =  isset($arg["lastname"]) ? $arg["lastname"] : 'lastname';
	$customer->email = $email;
	if(isset($arg["passwd"]) && !empty($arg["passwd"])){
		$customer->passwd =  md5(_COOKIE_KEY.trim($arg["passwd"]));

		error_log(' [Create] <[key:'.$customer->passwd .']>'.'<[Email:'.$email.']>'.'<[Pass:'.trim($arg["passwd"]).']>');
	}
	...
	$customer->identification = empty($arg["dni"]) ? $arg["dni"] : NULL;
	$customer->birthday =  empty($arg["birthday"]) ? $arg["birthday"] : NULL;
	$customer->website =  empty($arg["website"]) ? $arg["website"] : NULL;
	$customer->company =  empty($arg["company"]) ? $arg["company"] : NULL;
	$customer->id_type =  empty($arg["id_type"]) ? (int) $arg["id_type"] : 0;
