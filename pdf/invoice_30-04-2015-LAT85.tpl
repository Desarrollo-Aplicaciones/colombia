...
<b>N&uacute;mero de Pedido:</b>			{$order->id} {* getUniqReference() *}<br />
...
<table style="width: 100%; font-size: 8pt;">
				<tr style="line-height:4px; ">
					<td style="text-align: left; background-color: #3A9842; color: #FFF; padding-left: 10px; font-weight: bold; width: 15%; text-align:center">Referencia</td>
					...
					{if !$tax_excluded_display}
						<td style="background-color: #3A9842; color: #FFF; text-align: right; font-weight: bold; width: 10%">{l s='Unit Price' pdf='true'} <br />{*l s='(Tax Excl.)' pdf='true'*}</td>
					{/if}
					...
					<td style="background-color: #3A9842; color: #FFF; text-align: center; font-weight: bold; width: 5%">{l s='Qty' pdf='true'}</td>
				...
				{assign var="total_products_recalculado" value="0"}
				{foreach $order_details as $order_detail}
				{cycle values='#FFF,#DDD' assign=bgcolor}
				<tr style="line-height:6px;">
					<td style="text-align: left; width:15%">{$order_detail.product_reference}</td>
					...
					0
					{else}
						{if $recalculadoivaproducto}
							{assign var="iva_calc" value=$order_detail.iva_recalculado}
						{else}
					...
					{if $recalculadoivaproducto}
						{displayPrice currency=$order->id_currency price=($order_detail.unit_price_tax_excl+$order_detail.iva_recalculado)*$order_detail.product_quantity}
						{$product_total = ($order_detail.unit_price_tax_excl + $order_detail.iva_recalculado) * $order_detail.product_quantity}
						{$total_products_recalculado = $total_products_recalculado + $product_total}
					{else}
						{if $tax_excluded_display}
							{displayPrice currency=$order->id_currency price=$order_detail.iva_recalculado}
						{else}
            ...
            <table style="width: 100%">
				...
				<tr style="line-height:5px;">
					...
					{if $recalculadoivaproducto}
						{displayPrice currency=$order->id_currency price=$total_products_recalculado}</td>
					{else}
						{displayPrice currency=$order->id_currency price=$order_invoice->total_products}</td>
					{/if}
				</tr>
				{/if}
