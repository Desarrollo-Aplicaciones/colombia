...
class AdminProductsControllerCore extends AdminController
{
		...
		if ($result && Validate::isLoadedObject($this->object))
		{
			...
			$this->object->loadStockData();
              ...
              if($this->inBlackList==NULL && $opt)
                {
                     $this->inBlackList =$this->isBlackList();
                }		
                if($this->isModifiable === NULL){
                   $this->isModifiable = $this->is_modifiable (); 
                }
	...
	public function processStatus()
	{
        ...
        if (!$this->inBlackList && $this->isModifiable) {
	...
	public function initToolbar()
	{
		...
		if ($this->display == 'edit' || $this->display == 'add')
		{
			if ($product = $this->loadObject(true))
                        { 
                            if($this->inBlackList && $this->isBlackList() ) { $this->object->active =  0; }
                       
                             elseif (!$this->isModifiable ) {
                                  $this->object->active = 1;  
                                    $this->context->smarty->assign('notActiveProd', true);
                                }
...
WHERE prod.reference ='" . $this->object->reference. "';";
    ...
    protected function is_modifiable() {
...
FROM
(
select IF(orders.current_state IN (5,6,8,16),    IF(icr.id_estado_icr = 3 , 'ok', 'no')   ,  'no' ) as lista_negra
...
WHERE prod.reference = '" . $this->object->reference. "' and icr.id_estado_icr in(2,3)
