...
class AdminSupplyOrdersControllerCore extends AdminController
{
...

				{
...

					{
...

						{
							...
							$error ='';
							if($old_state->id == 1 && $new_state->id == 2 && Tools::isSubmit('id_supply_order')){
								...
								if (!empty($conn_ftp)){
									...
									if (is_array($status) && $status['status'] == 'ERROR'){
										...
										if(isset($status['errors'])){
											$error .= '<br>'.count($status['errors']).' errores.';
	...
	{ 
		...
		$items = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS($query); // echo $query->__toString();
